...
prepare() {
  ...
  # Dependency installation is intentionally outside the Safe Change timer.
  # The transaction snapshot must see a complete, package-owned /etc/ufw so a
  # rollback never leaves an installed UFW package with its config directory removed.
  have ufw || apt_install ufw || return 40
}
apply() {
  require_root || return 1; have ufw || { error "UFW 尚未准备好；请通过 vpsg firewall apply 运行"; return 20; }
  local p was_active=0 existed=0
  p="$(ssh_primary_port 2>/dev/null)" || { error "无法确认 SSH 端口，拒绝启用 UFW"; return 30; }
  plan || return $?; confirm "应用 UFW 防火墙配置？" || return 10
  ensure_runtime_dirs || return 40; ufw_active && was_active=1; rule_exists "$p" && existed=1
  printf 'port=%s\nwas_active=%s\nrule_existed=%s\n' "$p" "$was_active" "$existed" | atomic_write_file "$STATE" 600 || return 40
  ufw default deny incoming >/dev/null || return 40
  ufw default allow outgoing >/dev/null || return 40
  ((existed)) || ufw allow "$p/tcp" comment 'VPS Guard SSH' >/dev/null || return 40
  ufw --force enable >/dev/null || return 40
  verify || return 50
  log_event INFO "ufw applied ssh_port=$p"; ok "UFW 已启用并保留 SSH 端口 $p/tcp"
}
...
deny_rule() { require_root || return 1; local p="${1:-}" proto="${2:-tcp}" sshp; valid_port "$p" || return 64; [[ "$proto" =~ ^(tcp|udp)$ ]] || return 64; have ufw || apt_install ufw || return 40; sshp="$(ssh_primary_port 2>/dev/null || true)"; [[ "$p" != "$sshp" ]] || { error "拒绝阻断当前 SSH 端口 $p。请先迁移并验证 SSH。"; return 30; }; confirm "拒绝 $p/$proto 入站？" || return 10; ufw deny "$p/$proto"; log_event INFO "ufw deny port=$p proto=$proto"; }
delete_rule() { require_root || return 1; local p="${1:-}" proto="${2:-tcp}" sshp; valid_port "$p" || return 64; [[ "$proto" =~ ^(tcp|udp)$ ]] || return 64; have ufw || { error "UFW 未安装"; return 20; }; sshp="$(ssh_primary_port 2>/dev/null || true)"; [[ "$p" != "$sshp" ]] || { error "拒绝删除当前 SSH 端口规则"; return 30; }; confirm "删除 allow $p/$proto？" || return 10; ufw --force delete allow "$p/$proto"; log_event INFO "ufw delete allow port=$p proto=$proto"; }
...
case "$action" in status|check|list) status "$@";; prepare) prepare;; plan) plan;; apply) apply;; verify) verify;; rollback) rollback;; allow) allow_rule "$@";; deny) deny_rule "$@";; delete) delete_rule "$@";; web) web_profile;; doctor) plan >/dev/null && ok "UFW 前置检查通过" || exit 30;; *) error "firewall 支持: status|prepare|plan|apply|verify|rollback|allow|deny|delete|web"; exit 64;; esac
